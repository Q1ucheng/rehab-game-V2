 ...
 * 
 * 功能描述：
 * 负责管理康复训练数据的采集、存储和上传，实现固定频率的数据采样和实时传输
 * 
 * 技术栈：
 * - TypeScript
 * - WebSocket (与本地数据记录服务器通信)
 * - Firebase (数据存储)
 * - Singleton模式 (单例设计模式)
 * 
 * 主要功能模块：
 * 1. 训练数据结构定义 (TrainingData接口)
 * 2. WebSocket连接管理 (连接、重连、错误处理)
 * 3. 固定频率数据采样 (100Hz采样率)
 * 4. 数据缓冲区管理 (批量发送优化)
 * 5. 训练会话生命周期管理 (开始、结束、状态跟踪)
 * 6. 数据上传到Firebase (自动上传训练结果)
 * 
 * 核心特性：
 * - 100Hz固定采样频率，确保数据采集的准确性
 * - 实时WebSocket传输，支持数据流式处理
 * - 自动重连机制，保证连接稳定性
 * - 北京时间格式化，符合本地使用习惯
 * - 输入检测机制，智能判断有效训练数据
 * 
